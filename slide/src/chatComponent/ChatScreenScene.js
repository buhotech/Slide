...
class ChatScreenScence extends Component{
	...
	sendMessage(){
		...
		let username = "myguy";
		fetch('https://us-central1-project-bc489.cloudfunctions.net/slide/lilchat/chats/21/messages/new', {
  		method: 'post',
  		headers: {
	...
	componentDidMount(){
		let c_screen = this;
		firebase.database().reference("lilchat/chats/21/messages/").once("value", function(snap){
	...
	render(){
		...
		for(let message in this.state.messages){
			let temp = (<div><p>{this.state.messages[message].username} :</p><p>{this.state.messages[message].content}</p><br/></div>)
